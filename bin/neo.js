...
utils.genfileStoreName = function genfileStoreName(root, fileName) {
    ...
    const _fileName = isrelative ? path$2.basename(fileName) : fileName;
    ...
    let _content$ = ignoreDynamic ? _content : _content.replace(/(?<!\/\/[^\n]*)import\(['"`](\.?\.\/)?([\-\w\d\.\$\/@\}\{]+)['"`]\)/g,
        ...
        if (match) {
            ...
            if (((match[2] || match[4])?.length > 1) || isrelative) {
                ...
                const files = fs$1.readdirSync(isrelative ? root : (nodeModulesPath = findProjectRoot(this.pathMan.dirPath, globalOptions) + '/') + match[1] || '')
